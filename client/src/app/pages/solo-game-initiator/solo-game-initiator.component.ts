...
export class SoloGameInitiatorComponent implements OnInit {
    ...
    verifierNoms() {
        ...
        var temp: string = this.nomTemporaire.split(" ").join("").toLocaleLowerCase();
        this.assignerNomAdversaire();
        if (this.nomTemporaire.split(' ').join('').toLocaleLowerCase() === this.nomAdversaire.split(' ').join('').toLocaleLowerCase()) {
            ...
            this.nomEstValide = false;
        } else if (this.listeDesInsultes.search(temp.substring(0,3)) !== VERIFICATION_PRESENCE &&
        this.listeDesInsultes.search(temp.substring(4,7)) !== VERIFICATION_PRESENCE && this.listeDesInsultes.search(temp.substring(8,10)) !== VERIFICATION_PRESENCE && this.listeDesInsultes.search(temp.substring(11,13)) !== VERIFICATION_PRESENCE && this.listeDesInsultes.search(temp.substring(14,16)) !== VERIFICATION_PRESENCE){
            this.nomEstValide = false;
        } else {
